...
const Header = () => {
  const {user,logOut,admin}=useAuth();
    return (
        <div>
          <Navbar bg="dark" variant="dark" expand="lg" className='nav-bg'>
          ...
          {user?.email ?
                      ...
                      <Button onClick={logOut} className="btn btn-warning ms-3 me-3"> Logout</Button> :
                      <Nav.Link className="nav-link ms-3 me-3 btn btn-primary" as={Link} to="/signin"> Sign In</Nav.Link>}
        </NavDropdown>
