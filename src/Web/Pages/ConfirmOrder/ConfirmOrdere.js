...
const onSubmit = (data) => {
    ...
    return (
        <div>
          <Header></Header>
          <Container>
              <h1 className="mt-3 text-primary">Proceed To Confirm Order</h1>
          <div className="row mt-5 mb-5">
          <div className="servicedetails col-lg-4  rounded-3 shadow p-5">
          <h6 className="text-start text-light m-2"><span className="py-1 px-2 customround">{specificDetail?.category}</span></h6>
          <img className="w-50 mt-5" src={specificDetail?.spots_image} alt="" />

           <h5 className="mt-1 text-danger title-font">{specificDetail?.blog_title}</h5>
           <p className=" fw-lighter des-font">{specificDetail?.description}</p>
           <p className="title-font text-secondary">$ {specificDetail?.cost}</p>
           <p className="title-font text-secondary">{specificDetail?.destination}</p>
           <p className="title-font text-secondary">{specificDetail?.duration} Days</p>
          </div>
